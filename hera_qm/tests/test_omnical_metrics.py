...
def omnical_data():
    ...
    return OM, fc_file, fc_file_bad, fc_fileyy, oc_file, out_dir


def test_init(omnical_data):
    OM, fc_file, fc_file_bad, fc_fileyy, oc_file, out_dir = omnical_data
    assert OM.Nants == 17
    assert OM.filename == 'zen.2457555.42443.xx.HH.uvcA.good.omni.calfits'
    assert OM.omni_gains.shape == (17, 3, 1024, 1)


def test_load_firstcal_gains(omnical_data):
    OM, fc_file, fc_file_bad, fc_fileyy, oc_file, out_dir = omnical_data
    firstcal_delays, firstcal_gains, fc_pol = omnical_metrics.load_firstcal_gains(fc_file)
...
def test_omni_load_firstcal_gains(omnical_data):
    OM, fc_file, fc_file_bad, fc_fileyy, oc_file, out_dir = omnical_data
    OM.load_firstcal_gains(fc_file)
    assert hasattr(OM, 'gain_diff')
    # test exception
    pytest.raises(ValueError, OM.load_firstcal_gains, fc_fileyy)
...
def test_run_metrics(omnical_data):
    ...
    # no cut band
    full_metrics = OM.run_metrics(cut_edges=False)
    metrics = full_metrics['XX']
    assert metrics['ant_phs_std'] is None
    ...
    assert type(metrics) is OrderedDict
    # use fc file
    full_metrics = OM.run_metrics(fcfiles=fc_file)
    metrics = full_metrics['XX']
...
def test_write_load_metrics(omnical_data):
    ...
    outfile = os.path.join(out_dir, 'omnical_metrics.json')
    ...
    # erase
    os.remove(outfile)
    outfile = os.path.join(out_dir, 'omnical_metrics.pkl')
    if os.path.isfile(outfile):
        os.remove(outfile)
    # write pkl
    ...
    # write w/o filename
    outfile = os.path.join(OM.filedir, OM.filestem + '.omni_metrics.json')
    omnical_metrics.write_metrics(full_metrics, filetype='json')
    os.remove(outfile)
    outfile = os.path.join(OM.filedir, OM.filestem + '.omni_metrics.pkl')
...
def test_plot_phs_metrics(omnical_data):
    ...
    fname = os.path.join(OM.filedir, 'phs.png')
    ...
    omnical_metrics.plot_phs_metric(metrics, plot_type='std', fname=fname, save=True, outpath=OM.filedir)
    assert os.path.isfile(fname)
    os.remove(fname)
    omnical_metrics.plot_phs_metric(metrics, plot_type='hist', fname=fname, save=True, outpath=OM.filedir)
    assert os.path.isfile(fname)
    os.remove(fname)
    omnical_metrics.plot_phs_metric(metrics, plot_type='ft', fname=fname, save=True, outpath=OM.filedir)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # plot w/o fname
    basename = utils.strip_extension(OM.filename)
    fname = os.path.join(OM.filedir, basename + '.phs_std.png')
    if os.path.isfile(fname):
        os.remove(fname)
    omnical_metrics.plot_phs_metric(metrics, plot_type='std', save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    fname = os.path.join(OM.filedir, basename + '.phs_ft.png')
    if os.path.isfile(fname):
        os.remove(fname)
    omnical_metrics.plot_phs_metric(metrics, plot_type='ft', save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    fname = os.path.join(OM.filedir, basename + '.phs_hist.png')
    if os.path.isfile(fname):
        os.remove(fname)
    ...
    # exception
    del OM.omni_gains
    pytest.raises(Exception, OM.plot_gains)
    OM.run_metrics()
    # check return figs
...
def test_plot_chisq_metrics(omnical_data):
    ...
    full_metrics = OM.run_metrics(fcfiles=fc_file)
    metrics = full_metrics['XX']
    fname = os.path.join(OM.filedir, 'chisq.png')
    if os.path.isfile(fname):
        os.remove(fname)
    # plot w/ fname
    ...
    plt.close('all')
    # plot w/ fname and outpath
    omnical_metrics.plot_chisq_metric(metrics, fname=fname, save=True, outpath=OM.filedir)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # plot w/o fname
    fname = os.path.join(OM.filedir, utils.strip_extension(OM.filename)
                         + '.chisq_std.png')
...
def test_plot_chisq_tavg(omnical_data):
    OM, fc_file, fc_file_bad, fc_fileyy, oc_file, out_dir = omnical_data
    import matplotlib.pyplot as plt
    OM.run_metrics(fcfiles=fc_file)
    fname = os.path.join(OM.filedir, 'chisq_tavg.png')
    # test execution
    if os.path.isfile(fname):
        os.remove(fname)
    OM.plot_chisq_tavg(fname=fname, save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    OM.plot_chisq_tavg(ants=OM.ant_array[:2], fname=fname, save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # check return figs
...
def test_plot_gains(omnical_data):
    ...
    import matplotlib.pyplot as plt
    OM.run_metrics(fcfiles=fc_file)
    fname = os.path.join(OM.filedir, 'gains.png')
    if os.path.isfile(fname):
        os.remove(fname)
    # plot w/ fname
    OM.plot_gains(plot_type='phs', fname=fname, save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    OM.plot_gains(plot_type='amp', fname=fname, save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # divide_fc = True
    OM.plot_gains(plot_type='phs', fname=fname, save=True, divide_fc=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    OM.plot_gains(plot_type='amp', fname=fname, save=True, divide_fc=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # plot w/ fname and outpath
    OM.plot_gains(plot_type='phs', fname=fname, save=True, outpath=OM.filedir)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # plot w/o fname
    fname = os.path.join(OM.filedir, utils.strip_extension(OM.filename)
                         + '.gain_phs.png')
    OM.plot_gains(plot_type='phs', save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    fname = os.path.join(OM.filedir, utils.strip_extension(OM.filename)
                         + '.gain_amp.png')
    OM.plot_gains(plot_type='amp', save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # plot feeding ax object
    ...
    # plot feeding ants
    fname = os.path.join(OM.filedir, 'gains.png')
    OM.plot_gains(ants=OM.ant_array[:2], fname=fname, save=True)
    assert os.path.isfile(fname)
    os.remove(fname)
    plt.close('all')
    # check return figs
...
def test_plot_metrics(omnical_data):
    ...
    metrics = full_metrics['XX']
    basename = utils.strip_extension(OM.filename)
    fname1 = os.path.join(OM.filedir, basename + '.chisq_std.png')
    fname2 = os.path.join(OM.filedir, basename + '.phs_std.png')
    fname3 = os.path.join(OM.filedir, basename + '.phs_ft.png')
...
def omnicalrun_data():
    ...
    out_dir = os.path.join(DATA_PATH, 'test_output')
    return fc_file, oc_file, oc_filedir, oc_basename, out_dir


def test_firstcal_metrics_run(omnicalrun_data):
    fc_file, oc_file, oc_filedir, oc_basename, out_dir = omnicalrun_data
    # get arg parse
    ...
    # test w/ no fc_file
    arguments = ''
    cmd = ' '.join([arguments, oc_file])
    args = a.parse_args(cmd.split())
    history = cmd
    outfile = utils.strip_extension(oc_file) + '.omni_metrics.json'
    if os.path.isfile(outfile):
        os.remove(outfile)
    ...
    args = a.parse_args(cmd.split())
    history = cmd
    outfile = utils.strip_extension(oc_file) + '.omni.json'
    if os.path.isfile(outfile):
        os.remove(outfile)
    omnical_metrics.omnical_metrics_run(args.files, args, history)
    assert os.path.isfile(outfile)
    ...
    # test w/ metrics_path
    arguments = '--metrics_path={}'.format(out_dir)
    cmd = ' '.join([arguments, oc_file])
    args = a.parse_args(cmd.split())
    history = cmd
    outfile = os.path.join(out_dir, utils.strip_extension(oc_basename)
                           + '.omni_metrics.json')
    if os.path.isfile(outfile):
        os.remove(outfile)
    omnical_metrics.omnical_metrics_run(args.files, args, history)
    ...
    # test w/ options
    arguments = '--fc_files={0} --no_bandcut --phs_std_cut=0.5 --chisq_std_zscore_cut=4.0'.format(fc_file)
    cmd = ' '.join([arguments, oc_file])
    args = a.parse_args(cmd.split())
    history = cmd
    outfile = utils.strip_extension(oc_file) + '.omni_metrics.json'
    ...
    # test make plots
    arguments = '--fc_files={0} --make_plots'.format(fc_file)
    cmd = ' '.join([arguments, oc_file])
    args = a.parse_args(cmd.split())
    history = cmd
    omnical_metrics.omnical_metrics_run(args.files, args, history)
    basename = utils.strip_extension(oc_file)
