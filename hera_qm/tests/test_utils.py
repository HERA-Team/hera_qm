...
def test_get_pol():
    ...
    assert utils.get_pol(filename) == 'xx'
...
def test_gen_full_returns_input_for_full_pol_files_uvh5():
    ...
    assert sorted(expected_output_list) == sorted(output_files)
...
def test_gen_full_returns_fullpol_for_partial_pol_files():
    ...
    pytest.raises(ValueError, utils.generate_fullpol_file_list,
...
def test_generate_fullpol_file_list():
    ...
    assert sorted(fullpol_file_list[0]) == sorted(file_list)
    ...
    assert len(fullpol_file_list) == 1
    assert len(fullpol_file_list[0]) == 4
...
def test_get_metrics_ArgumentParser_ant_metrics():
    ...
    assert args.alwaysDeadCut == 10.0
...
def test_baselines_from_filelist_position(tmpdir):
    filelist = [str(tmp_path /  "test_input/zen.2458101.46106.xx.HH.OCR_53x_54x_only.first.uvh5"),
                str(tmp_path / "test_input/zen.2458101.46106.xx.HH.OCR_53x_54x_only.second.uvh5")]
