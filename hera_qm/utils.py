...
def lst_from_uv(uv):
    ...
    ----------
    uv
        A UVData or UVCal object.

    Returns
    -------
    lst_array
        An ndarray of LST values corresponding to the the time_array at the telecope
        location. Units are radians.
...
def collapse(a, alg, weights=None, axis=None, returned=False):
    ...
    alg : str
        Algorithm to use. Must be defined in this function with corresponding
        subfunction below.
    weights : array, optional
        The weights to be used for the collapse operation (e.g. weighted mean).
        NOTE: Some subfunctions do not use the weights. See corresponding doc strings.
    axis
        Axis or axes to collapse. Default is all. Must be an int or a tuple.
    returned : bool
        If True, return sum of weights in addition to combined array. Default is False.

    Returns
    -------
    out
        If returned is False, an array of the combined array. If returned is True,
        a 2-ple containing the combined array and the sum of the weights.
...
def mean(a, weights=None, axis=None, returned=False):
    ...
    This function is similar to np.average, except it handles infs (by giving them
    zero weight) and zero weight axes (by forcing result to be inf with zero output
    weight).
    ...
    weights
        The weights to use for averaging. If none, it will default to equal weight for
        all non-infinite data.
    axis
        The axis keyword to pass to np.sum.
    returned : bool
        If True, return sum of weights in addition to combined array. Default is False.
    ...
    wo
        If returned is True, the sum of the weights used to generate the average.
...
def absmean(a, weights=None, axis=None, returned=False):
    """Compute the average of the absolute value of data.
    ...
    axis
        The axis keyword to pass to np.mean.
    returned : bool
        If True, return sum of weights in addition to combined array. Default is False.
    ...
    -------
    o
        The average of the absolute value of the input data.
    wo
        If returned is True, the sum of the weights used to generate the average.
    """
...
def quadmean(a, weights=None, axis=None, returned=False):
    """Compute the average of data in quadrature.
    ...
    -------
    o
        The average in quadrature of the input data.
    wo
        If returned is true, the sum of the weights used to generate the average.
...
def or_collapse(a, weights=None, axis=None, returned=False):
    """Collapse axes using OR operation.

    Parameters
    ----------
    a
        The array of boolean values to process.
    weights
        NOT USED, but kept for symmetry with other averaging functions.
    axis
        The axis or axes over which to OR.
    returned : bool
        If True, return dummy weights array. NOTE: the dummy weights will simply be
        an array of ones. Default is False.

    Returns
    -------
    o
        The output of the OR operation on the input data.
    wo
        If returned is True, the dummy weights.
...
def and_collapse(a, weights=None, axis=None, returned=False):
    """Collapse axes using AND operation.
    ...
    weights
        NOT USED, but kept for symmetry with other averaging functions.
    axis
        The axis or axes over which to AND.
    returned : bool
        If True, return dummy weights array. NOTE: the dummy weights will simply
        be an array of ones. Default is False.

    Returns
    -------
    o
        The output of the AND operation on the input data.
    wo
        If returned is True, the dummy weights.
...
def and_rows_cols(waterfall):
    """Flag pixels only if fully flagged along time and/or frequency axes.

    For a 2D flag waterfall, this function flags pixels if they are totally flagged
    along the time and/or frequency axes. It assumes the shape of the input array
    is (Ntimes, Nfreqs).

    Parameters
    ----------
    waterfall
        2D boolean array of shape (Ntimes, Nfreqs).

    Returns
    -------
    wf
        A 2D array (size same as input) where only times/integrations that were fully
        flagged are flagged.
    """
...
def flags2waterfall(uv, flag_array=None, keep_pol=False):
    """Convert a full flag array to a 2D waterfall.

    This function converts an array of flags contained in a UVData or UVCal object
    into a single 2D waterfall of size (Ntimes, Nfreqs). To accomplish this, the
    function averages over baselines and polarizations (in the case of visibility
    data), or antennas and Jones parameters (in the case of calibration data).

    Parameters
    ----------
    uv
        A UVData or UVCal object which defines the times and frequencies, and
        supplies the flag_array to convert (if flag_array not specified)
    flag_array
        Optional flag array to convert instead of uv.flag_array. Note it must have
        the same dimensions as uv.flag_array.
    keep_pol : bool
        If True, keep the polarization axis intact. Default is False.

    Returns
    -------
    waterfall
        A 2D waterfall of averaged flags. For example, this is the fraction of baselines
        which are flagged for every time and frequency (in case of UVData input).
        The output size is (Ntimes, Nfreqs) or (Ntimes, Nfreqs, Npols).
