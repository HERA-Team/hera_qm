...
class RFI:
    def __init__(self,pyuvObject):
        """Takes in a pyuvdata object for administering various 
        RFI flagging strategies."""
        # Data must be prepped, excess dimensions clipped, polarizations
        # dealt with, etc
        self.uv = pyuvObject
        self.d = self.uv.data_array.squeeze()
        self.d = np.ma.masked_array(self.d,mask=np.isnan(self.d))
        self.f = self.uv.flag_array.squeeze()

    def flagstats(self,t=None,f=None):
        """Return statistics about flagged channels."""
        flagarr = self.f
        if t != None:
            return np.sum(flagarr[t,:])/(1.*len(flagarr[t,:]))
        if f != None:
...
>>>>>>> Added xrfi example class for comparison.
    ...
    def medminfilt(self,d, K=8, complex=False):
        ...
        if complex:
            for i in xrange(d.shape[0]):
                for j in xrange(d.shape[1]):
                    ...
                    d_sm[i, j] = self.medmin(d[i0:i1, j0:j1].real) + 1j*self.medmin(d[i0:i1, j0:j1].imag)
            return d_sm
        else:
            for i in xrange(d.shape[0]):
                for j in xrange(d.shape[1]):
                    i0, j0 = max(0, i - K), max(0, j - K)
                    i1, j1 = min(d.shape[0], i + K), min(d.shape[1], j + K)
                    d_sm[i, j] = self.medmin(d[i0:i1, j0:j1])
            return d_sm


    def watershed_flag(self, f=None, sig_init=6, sig_adj=2):
    ...
    def xrfi_simple(self, f=None, nsig_df=6, nsig_dt=6, nsig_all=0):
    ...
    def detrend_deriv(self, dt=True, df=True):
    ...
    def detrend_medminfilt(self, K=8, complex=False):
        ...
        d = self.d
        if complex:
            d_sm = self.medminfilt(np.abs(d)*np.angle(d), 2*K + 1)
            d_rs = d - d_sm
            d_sq = np.abs(d_rs)**2
            # puts minmed on same scale as average
            sig = np.sqrt(self.medminfilt(d_sq, 2 * K + 1,complex=True)) * (K / .64)
            f = d_rs / sig
            return f
        else:
            d_sm = self.medminfilt(np.abs(d), 2 * K + 1)
            d_rs = d - d_sm
            d_sq = np.abs(d_rs)**2
        # puts minmed on same scale as average
            sig = np.sqrt(self.medminfilt(d_sq, 2 * K + 1)) * (K / .64)
            f = d_rs / sig
            return f


    def detrend_medfilt(self, K=8):
        ...
        d_sm = self.medfilt(d, 2 * K + 1)
        d_rs = d - d_sm
        d_sq = np.abs(d_rs)**2
        # puts median on same scale as average
        # .456 scaling?
        sig = np.sqrt(self.medfilt(d_sq, 2 * K + 1) / .456)
        f = d_rs / sig
        return f[K:-K, K:-K]


    def xrfi(self, f=None, K=8, sig_init=6, sig_adj=2):
        ...
        """
        d = self.d
        nsig = self.detrend_medfilt(d, K=K)
        f = self.watershed_flag(np.abs(nsig), f=f, sig_init=sig_init, sig_adj=sig_adj)
        self.f = f
        return f
